...
namespace DriveClubFS.Utils;

public class PS4Swizzler
{
    public static void Swizzle(Span<byte> input, Span<byte> output, int width, int height, int pixelsPerBlock, int blockSize)
    {
        var heightTexels = height / pixelsPerBlock;
        var heightTexelsAligned = (heightTexels + 7) / 8;
        int widthTexels = width / pixelsPerBlock;
        ...
        for (int y = 0; y < heightTexelsAligned; y++)
        {
            for (int x = 0; x < widthTexelsAligned; x++)
            {
                for (int t = 0; t < 64; t++)
                {
                    int pixelIndex = PS4Swizzler.MortonReorder(t, 8, 8);
